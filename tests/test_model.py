...
def test_spiral_sigma_nonpositive():
    """sigma = -softplus(.) <= 0 for any state / control (fix #2)."""
    cfg, p = make("spiral")
...
def test_long_rollout_is_bounded(backbone):
    """Stability guarantees a long rollout cannot blow up (fix #2)."""
    cfg, p = make(backbone)
    ...
    # non-expansive: latent norm never exceeds initial norm by more than a hair
...
def test_spiral_radius_conditioning_changes_eigs():
    ...
    # dominant sigma and omega should differ across radii (not a flat map)
...
def test_encoder_jvp_matches_finite_difference():
    ...
    h = 1e-3   # central differences in float32: h too small amplifies rounding noise
...
def test_sobolev_normalisation_scales_present():
    ...
    assert xdot_scale[0] > xdot_scale[1]      # |dv/dt| spread > |dw/dt| spread
